...
## Contents API (File CRUD — No Git Required)
...
| Get repo tree | GET | `/repos/{owner}/{repo}/git/trees/{branch}?recursive=1` |

Note: PUT requires `content` as base64-encoded string and `message` as commit message.
Useful when `gh` and git push are not available (restricted environments like WSL without SSH keys).
