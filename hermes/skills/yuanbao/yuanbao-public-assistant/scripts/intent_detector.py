...
# === 意图模式表 ===
PATTERNS = [
    # 文件请求必须在写文案/写文章/做方案之前，防止被这些模式抢先匹配
    ("文件请求", [
        r'(传|发|上传|发送|给我|发给我|传给我).*(文档|文件|word|docx|md|方案|报告|文章)',
        r'(我要|我想要|给我).*(之前|上次|那个|那份).*(文档|文件|方案|报告|文章)',
        r'(把|将).*(之前|那个|那份).*(文档|文件|方案|报告|文章).*(传|发|上传|转)',
        r'(下载|导出|转成).*(word|文档|docx)',
        r'(.+)(的|打个).*(文档|word版)',
    ]),
    ("写文案", [
...
def extract_params(text):
    ...
    # 文件请求参数
    file_match = re.search(r'(之前|上次|那个|那份|文档|文件|方案|报告|文章|word|docx)\s*(?:的|叫|是)?\s*[「"《]?([^「"》\s,，。.!！?？]+)', text)
    if file_match:
        params['file_keyword'] = file_match.group(2)
    elif re.search(r'(传|发|上传).*(文档|文件|word|docx)', text):
        # 无具体文件名，传最近生成的
        params['file_keyword'] = 'latest'
    return params


def detect(text):
    """检测意图，返回 (intent_name, params)，未命中返回 (None, {})"""
    text = text.strip()
    for intent_name, pattern_list in PATTERNS:
        for pattern in pattern_list:
            if re.search(pattern, text):
                return (intent_name, extract_params(text))
    return (None, {})
...
def get_prompt(intent_name, params, user_text):
    ...
    prompts = {
        ...
        "整理数据": f"{base}\n用户要整理数据。输出清晰的表格或列表。",
        "文件请求": f"{base}\n用户要之前生成过的文档/方案/报告的Word版本。你需要：1)用 md_to_docx.py 的 find 功能找到对应md文件 2)调用 convert 转换为docx 3)用MEDIA:路径上传到群里。回复简洁：「已找到，转成Word发给你👇」然后附上MEDIA路径。如果找不到匹配文件，列出最近的文件让用户选。",
        "计算": f"{base}\n用户要计算。给答案+简要计算过程。",
