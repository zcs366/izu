...
- `flux-logo-generation.md` — Verified prompts and patterns for generating brand logo concepts with Flux FP8 (與京/美叶 session 2026-05-10)
...
- `flux_fp8_txt2img.json` — CheckpointLoaderSimple-based workflow for all-in-one
  Flux FP8 checkpoints (e.g. `Comfy Org-flux1-dev-fp8-with_clip_vae.safetensors`).
  Use this when `workflows/flux_dev_txt2img.json` (modular UNET/CLIP/VAE loaders)
  fails with 500 because the user has a combined model.
...
12. **All-in-one checkpoints vs modular workflows** — The skill's pre-built workflows
    (e.g. `flux_dev_txt2img.json`) use separate `UNETLoader` + `DualCLIPLoader` +
    `VAELoader` nodes. Many users (especially ComfyUI Desktop/Portable users) download
    **all-in-one FP8 checkpoints** like `Comfy Org-flux1-dev-fp8-with_clip_vae.safetensors`,
    which live in `models/checkpoints/` and require `CheckpointLoaderSimple` instead.
    **Symptoms:** 500 error on `/api/prompt`, ComfyUI server logs show no specific
    error (just "Error handling request"). **Fix:** Before running a workflow, check
    `models/checkpoints/` for all-in-one names containing `with_clip_vae`. If found,
    use `templates/flux_fp8_txt2img.json` (CheckpointLoaderSimple-based) instead of
    `workflows/flux_dev_txt2img.json`. Also check `models/unet/`, `models/vae/`,
    `models/clip/` separately — if the user has files in `checkpoints/` but NOT
    the expected split files in `unet/`/`vae/`/`clip/`, they almost certainly have
    an all-in-one model.

13. **Flux all-in-one template uses wrong latent node** — `templates/flux_fp8_txt2img.json`
    originally used `EmptySD3LatentImage` which is for SD3/SD3.5, NOT Flux. Flux requires
    `EmptyLatentImage`. **Symptoms:** workflow submits but produces black images or
    wrong aspect ratio outputs. **Fix:** The template has been patched (2026-05-10) to use
    `EmptyLatentImage`. If you see a workflow with `EmptySD3LatentImage` for a Flux model,
    replace it with `EmptyLatentImage`.
    **Also:** Flux uses cfg=1 (disables CFG, uses Flux's internal guidance instead).
    cfg=3.5 (SD default) will produce over-saturated/washed-out results.
    Negative prompt can be left empty for Flux — it doesn't process CFG negatives the
    same way as SD.

14. **`run_workflow.py` 500 error on modular workflows** — When `run_workflow.py`
    returns HTTP 500 with body "Server got itself in trouble", the most common
    cause is a node in the workflow referring to a model file that doesn't exist
    on the local disk. The skill scripts don't surface the ComfyUI server error
    details. Diagnosis:
    a. Check `models/unet/`, `models/vae/`, `models/clip/` for the required files
    b. Check `models/checkpoints/` for all-in-one alternatives
    c. Read `user/comfyui.log` or `user/comfyui.prev.log` in the ComfyUI install
       directory for actual error traces
    d. If log shows nothing specific, try a minimal smoke test (CheckpointLoaderSimple
       + KSampler + SaveImage) via direct API to distinguish server health from
       workflow-specific failure

15. **Flux first-run timeout** — The first workflow run after server start (or first
    run using a Flux model) can take 30-90 seconds just to load the model into VRAM.
    The default `run_workflow.py` timeout is 300s, which is sufficient, but if the
    script was invoked with a shorter timeout (e.g. 60s by an outer tool), it will
    time out before the model finishes loading. **Fix:** Always use `--timeout 300`
    (or more) for Flux runs, especially the first run. Subsequent runs (model already
    cached) are much faster at ~10-30s for 30 steps.

16. **WSL2→Windows localhost flaky** — When ComfyUI is running on Windows and the
    agent is running inside WSL2, `curl http://127.0.0.1:8188/...` may work for
    simple requests but consistently **times out when piped to `python3 -c`** for
    JSON processing, or when submitting workflow prompts via the REST API. This is
    a WSL2 networking issue (localhostRelay unreliability, firewall interference,
    or UNC path CWD problems when launching via `cmd.exe /c`). **Symptoms:**
    - Simple `curl` succeeds (`Connected to 127.0.0.1:8188`) but `curl | python3`
      hangs/times out
    - `cmd.exe /c "cd /d X: && start /B ..."` fails with UNC path errors when
      CWD is `\\wsl.localhost\...`
    - `python3 run_workflow.py --workflow ...` gets blank response or timeout
    **Workarounds (try in order):**
    1. Write a `.bat` file to the Windows filesystem (e.g. `I:\hermes\run.bat`)
       and ask the user to double-click it from File Explorer
    2. Write a Python script to the Windows filesystem and have the user run it
       with the Windows Python (`python.exe`)
    3. `cmd.exe /c "pushd I:\\ComfyUI\\... && start /B python.exe main.py"` —
       use `pushd` (UNC-aware) instead of `cd /d`
    4. Use `netsh` to check if Windows firewall blocks the port:
       `cmd.exe /c "netsh advfirewall firewall show rule name=all dir=in | findstr 8188"`
    **Long-term fix:** Add `[wsl2] localhostForwarding=true` to Windows
    `%UserProfile%\.wslconfig` and restart WSL.

17. **Flux VRAM sizing (RTX 2080 Ti 22GB)** — Measured VRAM usage for different
    Flux model variants on RTX 2080 Ti (22GB total, ~15.5GB free after boot):
    | Model variant | Approx VRAM | Works on 2080 Ti? |
    |--------------|-------------|-------------------|
    | `flux1-dev-fp8-with_clip_vae.safetensors` (17.2GB, combined ckpt) | 18-20GB | ❌ Crashes server |
    | `Kijai-flux1-dev-fp8-e4m3fn.safetensors` (11.9GB, UNET-only) + CLIP+VAE | 16-18GB | ⚠️ Marginal |
    | `flux1-dev-Q8_0.gguf` (12.7GB, UNET-only) + CLIP+VAE | 16-18GB | ⚠️ Marginal |
    | `flux1-dev.safetensors` (23.8GB, full fp16 UNET) | 24-28GB | ❌ Out of memory |
    | `int4 flux dev/` (6.6GB, quantized UNET) + fp8 CLIP + VAE | ~11GB | ✅ Stable |
    
    **Recommendation for 2080 Ti:** Use the int4 quantized Flux model
    (`models/diffusion_models/int4 flux dev/`) with t5xxl_fp8 CLIP (4.9GB) and
    flux_ave VAE (335MB). Total VRAM ~11GB, leaving headroom for tensor operations.
    **Workflow setup:** int4 models use `UNETLoader` (not `CheckpointLoaderSimple`),
    `DualCLIPLoader` for CLIP-L + T5-XXL, and `VAELoader`. The `comfy_config.json`
    inside the int4 model dir specifies Flux architecture parameters.
    
18. **ComfyUI 500 error with no log output** — When submitting a workflow via the REST
    API and getting HTTP 500 with "Server got itself in trouble" but the ComfyUI server
    log (`user/comfyui.log`) shows nothing specific, the cause is almost always:
    - A model file referenced by the workflow doesn't exist on disk (especially common
      when switching between all-in-one checkpoints and modular UNET/CLIP/VAE setups)
    - The workflow uses the wrong loader type (e.g. `CheckpointLoaderSimple` for an
      int4 model that needs `UNETLoader`)
    - **Check first:** List available models in the relevant folder
      (`models/checkpoints/`, `models/unet/`, `models/diffusion_models/`,
      `models/vae/`, `models/clip/`) and confirm the workflow references the
      exact filename (case-sensitive).
  
19. **Flux int4 workflow structure** — When using the int4 quantized Flux model
    (in `models/diffusion_models/int4 flux dev/`), the correct workflow is:
    ```
    UNETLoader(model_name="int4 flux dev", weight_dtype="default")
      → model output
    CLIPLoader(clip_name="t5xxl_fp8_e4m3fn.safetensors", type="stable_diffusion")
      → CLIP output  
    DualCLIPLoader(clip_name1="clip_l.safetensors", clip_name2="...", type="flux")
      → combined CLIP → used for CLIPTextEncode
    VAELoader(vae_name="flux_ave.safetensors")
      → VAE output
    KSampler(model=UNET, positive=CLIP, negative=CLIP, ...)
      → For Flux, use cfg=1 (disables CFG — Flux uses internal guidance) and
        scheduler="simple". Negative prompt can be empty for Flux.
    VAEDecode(samples=KSampler, vae=VAE)
    SaveImage(images=VAEDecode)
    ```
    Flux does NOT use `EmptySD3LatentImage` — use standard `EmptyLatentImage`.
